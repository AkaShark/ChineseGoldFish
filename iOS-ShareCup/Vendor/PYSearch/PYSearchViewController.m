...
@property (nonatomic,strong) UIButton *emBtn;
...
- (void)setup
{
     ...
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(upDateBasetable) name:@"hotAdArray" object:nil];
...
//    searchBar.backgroundColor = [UIColor cyanColor];
    ...
    _emBtn = [[UIButton alloc] init];
    [_emBtn setTitleColor:PYTextColor forState:UIControlStateNormal];
    [_emBtn setTitle:[NSBundle py_localizedStringForKey:PYSearchEmptyButtonText] forState:UIControlStateNormal];
    [_emBtn setImage:[NSBundle py_imageNamed:@"empty"] forState:UIControlStateNormal];
    [_emBtn addTarget:self action:@selector(emptySearchHistoryDidClick) forControlEvents:UIControlEventTouchUpInside];
    _emBtn.py_width += PYSEARCH_MARGIN;
    _emBtn.py_height += PYSEARCH_MARGIN;
    _emBtn.py_centerY = self.searchHistoryHeader.py_centerY+6;
    _emBtn.py_x = KScreenWidth-30 - _emBtn.py_width;
    [hotSearchView addSubview:titleLabel];
    [hotSearchView addSubview:_emBtn];
...
- (void)layoutForDemand {
    if (self.swapHotSeachWithSearchHistory == YES) { // 默认布局，热门搜索在搜索历史上方
...
- (void)setHotSearches:(NSArray *)hotSearches
{
    ...
    _hotSearches = self.searchHistories;
...
//
//    }
    self.hotSearches = self.searchHistories;
    if (self.searchHistories.count == 0)
    {
        _emBtn.hidden = YES;
    }
// 取消掉头部 刷新tableView恢复位置
    self.baseSearchTableView.tableHeaderView.frame = CGRectZero;
    [self.baseSearchTableView reloadData];
...
- (void)saveSearchCacheAndRefreshView
{
    ...
    if (searchText.length > 0) { // 只要显示搜索历史且不为空串才会缓存
        // 先移除再刷新
...
//            self.searchHistoryStyle = self.searchHistoryStyle;
//        }
//        更新下历史 这个干了错误 这个应该当时做的时候换好了 换成历史就好了。
        self.hotSearches = self.searchHistories;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 创建cell
    GFSearchTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (!cell) {
        cell = [[GFSearchTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
...
//        [cell.contentView addSubview:line];
    ...
    cell.callBack = ^(NSString * _Nonnull str) {
        GFShowAllClassViewController *vc = [[GFShowAllClassViewController alloc] init];
        vc.varityName = str;
        vc.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:vc animated:YES];
    };
    cell.dataArray = self.hotAdArray;
    cell.imageArray = self.hotImage;
    // 设置数据
...
{
//    return self.searchHistories.count && self.showSearchHistory && self.searchHistoryStyle == PYSearchHistoryStyleCell ?  : 0.01;
    return 25;
}
