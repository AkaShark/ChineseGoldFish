...
//    [itemButton setTitleColor:self.configration.normalItemColor forState:UIControlStateNormal];
    ...
    itemButton.tag = idx;
    itemButton.backgroundColor = [UIColor colorWithHexString:@"#158AFF"];
...
- (void)setupOtherViews {
    ...
    self.scrollView.frame = CGRectMake(0, 20, self.configration.showAddButton ? self.yn_width - self.yn_height : self.yn_width, KScreenHeight-25);
    ...
    if (scrollSizeWidht < self.scrollView.yn_width) {//不超出宽度
        ...
        if (self.configration.aligmentModeCenter && left >= 0) {
            ...
            self.scrollView.contentSize = CGSizeMake(left + CGRectGetMaxX([[self.itemsArrayM lastObject] frame]), KScreenHeight-25);
            ...
            if (!self.configration.scrollMenu) {
                ...
                self.scrollView.contentSize = CGSizeMake(CGRectGetMaxX([[self.itemsArrayM lastObject] frame]), KScreenHeight-25);
                
            } else {
                self.scrollView.contentSize = CGSizeMake(scrollSizeWidht, KScreenHeight-25);
            }
        }
