...
- (void)initSetButtonsView
{
    ...
    [btn_1 addTarget:self action:@selector(clickTheBtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [btn_2 addTarget:self action:@selector(clickTheBtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [btn_3 addTarget:self action:@selector(clickTheBtn:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)clickTheBtn: (UIButton *)sender
{
    if (sender.tag == 0)
    {
        ...
        NSNotification * notice = [NSNotification notificationWithName:@"btn" object:nil userInfo:@{@"type":@"cancel"}];
        //发送消息
    ...
    {
        NSNotification * notice = [NSNotification notificationWithName:@"btn" object:nil userInfo:@{@"type":@"many"}];
        //发送消息
        [[NSNotificationCenter defaultCenter]postNotification:notice];
    }
    else
    {
        NSNotification * notice = [NSNotification notificationWithName:@"btn" object:nil userInfo:@{@"type":@"four"}];
        //发送消息
        [[NSNotificationCenter defaultCenter]postNotification:notice];
    }
}
